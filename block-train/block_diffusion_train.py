...
from block_diffusion_config import BlockDiffusionConfig
...
def configure_wandb_env(args: BlockDiffusionConfig):
    ...
    os.environ.setdefault("WANDB_PROJECT", "block-diffusion")
    if getattr(args, "run_name", None):
        os.environ.setdefault("WANDB_NAME", str(args.run_name))


def parse_args() -> BlockDiffusionConfig:
    parser = HfArgumentParser(BlockDiffusionConfig)
...
def get_torch_dtype(args: BlockDiffusionConfig):
    if args.bf16:
...
def load_model_and_tokenizer(args: BlockDiffusionConfig):
...
def main():
    ...
    if args.gradient_checkpointing:
        raise ValueError(
            "block-train currently requires `gradient_checkpointing=False` because block diffusion depends on KV cache."
        )
